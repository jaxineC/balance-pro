...
import { ModalButton } from "../Button/Button.styled";
import { Modal } from "./SignUpModal.styled.js";

function SignUpModal({ isSignUp, setIsSignUp, setUserID }) {
	...
	async function initUserFirestore(user) {
		try {
			...
			await setDoc(doc(db, user.uid, "welcomeTXT"), {
				txt1: "Greeting text.",
				txt2:
					"Enter your own everyday greeting text above. And some details here. --- By click, type, and enter!",
			});
